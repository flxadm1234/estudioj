...
export const adminApi = {
  getSiteConfig() {
    return adminFetch<SiteConfig>("/admin/site-config");
  },
  ...
  getHero() {
    return adminFetch<HeroSection>("/admin/hero");
  },
  ...
  createService(body: Partial<Service> & { name: string; description: string }) {
    return adminFetch<Service>("/admin/services", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(body)
    });
  ...
  patchService(id: string, body: Partial<Service> & Record<string, unknown>) {
    return adminFetch<Service>(`/admin/services/${id}`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(body)
    });
  ...
  deleteService(id: string) {
    return adminFetch<void>(`/admin/services/${id}`, { method: "DELETE" });
  },
  listTeam() {
  ...
  createTeam(body: Partial<TeamMember> & { name: string }) {
    return adminFetch<TeamMember>("/admin/team", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(body)
    });
  ...
  patchTeam(id: string, body: Partial<TeamMember> & Record<string, unknown>) {
    return adminFetch<TeamMember>(`/admin/team/${id}`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(body)
    });
